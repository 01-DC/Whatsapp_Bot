        ...
        const isUrl = new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/gi)
			...
			//console.log(type)
                        if (isMedia && type == 'video') {
                  		...
                  		client.reply(from, 'You did not tag a picture or video!', message.id)
                    	}
		break
       case 'toimg':
       	if(!quotedMsg) return client.reply(from, '.', id)
		else if (quotedMsg && quotedMsg.type == 'video'){
		return client.reply(from, 'that\'s not a sticker!', id)
		} if(quotedMsg) {
	const mediaData = await decryptMedia(quotedMsg)
	const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
...
*Link*: ${url}`
        ...
        case 'pokemon':
            arg = body.trim().split(' ')
            if (arg.length < 2) {
            client.reply(from, 'Give me a pokemon name!', id)
	    } else {
        ...
        case 'link' :
            ...
            break
        case 'git' :
            return client.sendText(from, '*You can contribute to this Bot on Github at the link below.*\nAlso don\'t forget to star this Repo and follow me.\nThank you :-)\n\nhttps://github.com/01-DC/Whatsapp_Bot')
            break
        default:
